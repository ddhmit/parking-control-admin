...
describe('getRouteAuthority tests', () => {
  it('should return authority for each route', (): void => {
    const routes = [
      { path: '/user', name: 'user', authority: [UserRole.user], exact: true },
      { path: '/admin', name: 'admin', authority: [UserRole.admin], exact: true },
    ];
    expect(getRouteAuthority('/user', routes)).toEqual([UserRole.user]);
    expect(getRouteAuthority('/admin', routes)).toEqual([UserRole.admin]);
  });

  it('should return inherited authority for unconfigured route', (): void => {
    const routes = [
      { path: '/nested', authority: [UserRole.admin, UserRole.user], exact: true },
      { path: '/nested/user', name: 'user', exact: true },
    ];
    expect(getRouteAuthority('/nested/user', routes)).toEqual([UserRole.admin, UserRole.user]);
  });

  it('should return authority for configured route', (): void => {
    const routes = [
      { path: '/nested', authority: [UserRole.admin, UserRole.user], exact: true },
      { path: '/nested/user', name: 'user', authority: [UserRole.user], exact: true },
      { path: '/nested/admin', name: 'admin', authority: [UserRole.admin], exact: true },
    ];
    expect(getRouteAuthority('/nested/user', routes)).toEqual([UserRole.user]);
    expect(getRouteAuthority('/nested/admin', routes)).toEqual([UserRole.admin]);
  });

  it('should return authority for substring route', (): void => {
    const routes = [
      { path: '/nested', authority: [UserRole.user, 'users'], exact: true },
